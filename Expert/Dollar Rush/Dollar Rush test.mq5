...
void RunStrategy(int idx) {
    ...
    if (dt.day_of_year != states[idx].lastCalcDayOfYear ||
        ...
        PivotLevels newPivots = getPivotLevels(symbol, point, 0);
        if (newPivots.success) {
            states[idx].cachedPivots = newPivots;
            states[idx].lastCalcDayOfYear = dt.day_of_year;
            ArrayInitialize(states[idx].levelUsed, false);
            if (symbol == _Symbol) updateChartGraphics(idx, true);
        } else {
            return;
        }
    }
    ...
    if (symbol == _Symbol) updateChartGraphics(idx, false);
...
void LoadSettings(int i, string sym, int magic, bool enBuy, bool enSell,
                  double gridStep, int maxE, double r1g, double r2g, double r3g,
...
void LoadState(int i, string sLots, string bLots, double c2, double c3,
    ...
    ArrayInitialize(states[i].levelUsed, false);
}
...
void checkEntry(int idx, PivotLevels& pv, bool isDailyMode, double ask,
    ...
    for (int k = 0; k < 4; k++) {
        ...
        processLevel(idx, k, level, settings[idx].gaps[k], point, d, buyCount,
                     sellCount, ask, bid);
    }
    for (int k = 4; k < 8; k++) {
...
bool isTradeTooClose(int idx, ENUM_POSITION_TYPE type, double targetPrice,
                     double point) {
    double minGridDist = settings[idx].gridStep * point;
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        if (posInfo.SelectByIndex(i)) {
            if (posInfo.Symbol() == settings[idx].symbol &&
                posInfo.Magic() == settings[idx].magic &&
                posInfo.PositionType() == type) {
                double dist = MathAbs(posInfo.PriceOpen() - targetPrice);
                if (dist < minGridDist) return true;
            }
        }
    }
    return false;
...
bool checkNextDayGap(int idx, ENUM_POSITION_TYPE type, double currentPrice,
    ...
    if (type == POSITION_TYPE_BUY) {
        if (settings[idx].nextDayGapBuy != 0 &&
            diffPoints > settings[idx].nextDayGapBuy)
            return false;
    }
    if (type == POSITION_TYPE_SELL) {
        if (settings[idx].nextDayGapSell != 0 &&
            diffPoints < settings[idx].nextDayGapSell)
            return false;
    }
    return true;
}

void processLevel(int idx, int levelKey, double levelPrice, double alphaPoint,
                  double point, ENUM_TRADE_DIR dirConfig, int bCnt, int sCnt,
                  double ask, double bid) {
    if (dirConfig == DIR_NO_TRADE) return;
    if (states[idx].levelUsed[levelKey]) return;

    long currentSpread = SymbolInfoInteger(settings[idx].symbol, SYMBOL_SPREAD);
    if (currentSpread > MaxSpreadPoints) return;

    double targetPrice = levelPrice + (alphaPoint * point);
    double maxDist = MaxEntryDistance * point;

    if ((dirConfig == DIR_BUY || dirConfig == DIR_BOTH) &&
        settings[idx].enBuy && bCnt < settings[idx].maxEntries) {
        if (ask <= targetPrice && ask >= targetPrice - maxDist) {
            if (isTradeTooClose(idx, POSITION_TYPE_BUY, targetPrice, point))
                return;
            if (!checkNextDayGap(idx, POSITION_TYPE_BUY, ask, point)) return;

            double lot = getLotSize(idx, bCnt, POSITION_TYPE_BUY);
            if (trade.Buy(lot, settings[idx].symbol, 0, 0, 0,
                          "PG_S" + IntegerToString(idx + 1))) {
                if (EnableSoundAlerts) PlaySound("ok.wav");
                states[idx].levelUsed[levelKey] = true;
            }
        }
    }
    ...
    if ((dirConfig == DIR_SELL || dirConfig == DIR_BOTH) &&
        ...
        if (bid >= targetPrice && bid <= targetPrice + maxDist) {
            if (isTradeTooClose(idx, POSITION_TYPE_SELL, targetPrice, point))
                return;
            if (!checkNextDayGap(idx, POSITION_TYPE_SELL, bid, point)) return;

            double lot = getLotSize(idx, sCnt, POSITION_TYPE_SELL);
            if (trade.Sell(lot, settings[idx].symbol, 0, 0, 0,
                           "PG_S" + IntegerToString(idx + 1))) {
                if (EnableSoundAlerts) PlaySound("ok.wav");
                states[idx].levelUsed[levelKey] = true;
            }
...
void ResetLevelLocks(int idx) { ArrayInitialize(states[idx].levelUsed, false); }
...
void CalculatePivotsForce(int idx) {
    ...
    double point = SymbolInfoDouble(symbol, SYMBOL_POINT);
    if (point == 0) return;
    states[idx].cachedPivots = getPivotLevels(symbol, point, 0);
...
color GetDirColor(int idx, int lineKey, bool isDaily, color defaultColor) {
    ENUM_TRADE_DIR d = isDaily ? settings[idx].dailyDirs[lineKey]
                               : settings[idx].dirs[lineKey];
    if (d == DIR_NO_TRADE) return clrMediumPurple;
    return defaultColor;
}
...
void CleanupHistory(int idx) {
    ...
    for (int i = total - 1; i >= 0; i--) {
        ...
        if (StringFind(name, states[idx].prefix) < 0) continue;

        if (StringFind(name, "_R") > 0 || StringFind(name, "_S") > 0 ||
            StringFind(name, "_OP") > 0) {
            datetime objTime =
                (datetime)ObjectGetInteger(0, name, OBJPROP_TIME);
            if (objTime < cutoff) ObjectDelete(0, name);
        }
    }
}

void updateChartGraphics(int idx, bool force) {
    if (IsOptimization) return;
    if (MQLInfoInteger(MQL_TESTER) && !MQLInfoInteger(MQL_VISUAL_MODE)) return;
    ...
    for (int d = 0; d <= loopLimit; d++) {
        PivotLevels pv = getPivotLevels(settings[idx].symbol, point, d);
        if (!pv.success) continue;

        bool isDaily = (pv.rangeR4S4 < settings[idx].dailyMinRange);
        string dateSuffix = "_" + TimeToString(pv.dayStart, TIME_DATE) + "_";

        color cR4 = GetDirColor(idx, 3, isDaily, clrDodgerBlue);
        color cR3 = GetDirColor(idx, 2, isDaily, clrDodgerBlue);
        color cR2 = GetDirColor(idx, 1, isDaily, clrDodgerBlue);
        color cR1 = GetDirColor(idx, 0, isDaily, clrDodgerBlue);
        color cS1 = GetDirColor(idx, 4, isDaily, clrRed);
        color cS2 = GetDirColor(idx, 5, isDaily, clrRed);
        ...
        createDailyLine(p + dateSuffix + "R3", pv.dayStart, pv.dayEnd, pv.r3,
                        cR3, PivotLineStyle);
        createDailyLine(p + dateSuffix + "R2", pv.dayStart, pv.dayEnd, pv.r2,
                        cR2, PivotLineStyle);
        ...
        createDailyLine(p + dateSuffix + "S1", pv.dayStart, pv.dayEnd, pv.s1,
                        cS1, PivotLineStyle);
        createDailyLine(p + dateSuffix + "S2", pv.dayStart, pv.dayEnd, pv.s2,
                        cS2, PivotLineStyle);
        createDailyLine(p + dateSuffix + "S3", pv.dayStart, pv.dayEnd, pv.s3,
                        cS3, PivotLineStyle);
        createDailyLine(p + dateSuffix + "S4", pv.dayStart, pv.dayEnd, pv.s4,
                        cS4, PivotLineStyle);
